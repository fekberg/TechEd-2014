    ...
    public class SyncHandler : IMobileServiceSyncHandler
    {
        public async Task<JObject> ExecuteTableOperationAsync(IMobileServiceTableOperation operation)
        {
            while (true)
            {
                ...
                var localValue = operation.Item.ToObject<Customer>();
                var remoteValue = error.Value;

                var conflictResolutionDialog = new MessageDialog("How shall we proceed?\r\n\r\n" +
                    "Local Item: " + localValue.Firstname + "\r\n" +
                    "Remote Item: " + remoteValue.ToObject<Customer>().Firstname,
                    "Conflict detected! "
                );
                ...
                {
                    operation.Item[MobileServiceSystemColumns.Version] = remoteValue[MobileServiceSystemColumns.Version];
